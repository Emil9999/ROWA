  ...
  <v-stepper v-model="e1" class="step-number">
        ...
        <v-row justify="center" align-center>
          <v-col cols="2" style="padding-left: 30px">
            <v-btn
              ...
              color="white"
              @click="e1 -=1"
              v-on:click="videoInst=false, abortBlinking()"
            >
              <v-icon color="primary">mdi-arrow-left</v-icon>
            </v-btn>
          </v-col>
    ...
    <v-stepper-items color="secondary">
      ...
      <v-stepper-content step="2">
        <Plant_3
          v-on:gotoInstructions="gotoInstructions($event)"
...
export default {
  ...
  methods: {
    getPositonAndModuleOfPlant: function() {
      axios
        .get(
          "http://127.0.0.1:3000/plant/get-position",
          {
            params: {
    ...
    startBlinking: function() {
      axios
        .get("http://127.0.0.1:3000/light/" + this.moduleNum + "/breath-on")
        .then()
        .catch(error => {
          console.log(error);
        });
    ...
    abortBlinking: function() {
      axios
        .get("http://127.0.0.1:3000/light/" + this.moduleNum + "/breath-off")
        .then();
      this.$store.dispatch("clear_farming").catch(error => {
        console.log(error);
      });
    },
